...
public int calculateCapacity(Cycle c) {
	...
	for(Tuple t: c.itemPerCycle) {
		capacity_used += infra.sizeTelemetryItems[t.second];
	}
		...
		if(!leastUsed.itemPerCycle.isEmpty()) {                                     //se o ciclo foi usado para coletar algum item
			...
			for(Tuple t : leastUsed.itemPerCycle) {                                 //itero sobre todos os itens do ciclo
				...
				for(int i = 0; i < cycles_aux.size(); i++) {                        //itera sobre todos os outros ciclos
					if(cycles_aux.get(i).nodes.contains(t.first)) {				//se for encontrado um ciclo com capacidade sobrando que passe pelo mesmo nodo que o ciclo atual
						if(cycles_aux.get(i).capacity_used + this.infra.sizeTelemetryItems[t.second] <= cycles_aux.get(i).capacity && !cycles_aux.get(i).equals(leastUsed_aux)) {
							...
							cycles_aux.get(i).capacity_used += this.infra.sizeTelemetryItems[t.second];   //adiciono na capacidade
				...
				if(!found) {                                                        //procura por um ciclo que passe perto de um nodo utilizado pelo pathAtual, e o utiliza para coletar esse item
					...
					for(int c = 0; c < cycles_aux.size(); c++) {                    //itera sobre todos os ciclos
						...
						for(int i = 0; i < cycles_aux.get(c).nodes.size()-1; i++) { //itera sobre todos os nodos do ciclo
							int cost_dijkstra_ida = infra.getShortestPath(cycles_aux.get(c).nodes.get(i), t.first).size() - 1;   //calcula o tamanho do caminho de ida
							int cost_dijkstra_volta = infra.getShortestPath(t.first, cycles_aux.get(c).nodes.get(i)).size() - 1; //calcula o tamanho do caminho de volta(é o mesmo do caminho de ida)
							...
							int cost_node = cost_dijkstra_ida + cost_dijkstra_volta + infra.sizeTelemetryItems[t.second];           //calcula o caminho total para coletar o item partindo do nodo i
					...
					if(!found_dijkstra) {							                 //se não foi encontrado um ciclo que possa coletar esse item
						...
						ArrayList<Integer> caminho_ida = infra.getShortestPath(cycles_aux.get(choosed_cycle).nodes.get(choosed_device_index), t.first);      //armazena o caminho de ida
						...
						ArrayList<Integer> caminho_volta = infra.getShortestPath(t.first, cycles_aux.get(choosed_cycle).nodes.get(choosed_device_index));    //armazena o caminho de volta
